...
public class Main {
    ...
    public static String[] nombresArray = {"ANA", "LUIS", "CARLOS", "MARTA"};
    ...
    public static void iniciarBiblioteca() {
        ...
        TesisDigital tesis = new TesisDigital("Inteligencia Artificial en la Medicina", "TD-123", "Ciencia y Tecnología", "https://repositorio.universidad.edu/tesisIA", "Dr. Juan Pérez", "Universidad Nacional de Ciencia y Tecnología", 2025);

        Usuario usuario1 = new Usuario("Pablo", 1, 2);
        Usuario usuario2 = new Usuario("Manuela", 2, 3);

        listaBiblioteca.add(audioLibro);
        listaBiblioteca.add(dvd);
        listaBiblioteca.add(libroFisico);
        listaBiblioteca.add(revistaDigital);
        listaBiblioteca.add(tesis);
    ...
    public static Usuario buscarUsuario(int codigo) {
        ...
        for (int i = 0; i < listaUsuarios.size(); i++) {
            if (codigo == listaUsuarios.get(i).getIdUsuario()) {
                usuarioBuscado = listaUsuarios.get(i);
            }
        }
    ...
    public static void prestarMaterial() {
        ...
        if (materialBuscado != null) {
            if (materialBuscado instanceof Prestamo) {
                ...
                int codigoUsuario = miScanner.nextInt();
                miScanner.nextLine();

                Usuario usuarioBuscado = buscarUsuario(codigoUsuario);
                if (usuarioBuscado != null) {
                    Prestamo materialBuscadoPrestable = (Prestamo) materialBuscado;

                    if (materialBuscadoPrestable.reservable()) {
                        materialBuscadoPrestable.prestar(usuarioBuscado);
                        System.out.println(materialBuscado.getTitulo() + ", con codigo: " + materialBuscado.getCodigo() + ", ha sido prestado.");
                    } else {
                        System.out.println("No se puede prestar. El material ya está reservado.");
                    }
    ...
    public static void devolverMaterial() {
        ...
        if (materialBuscado != null) {
            if (materialBuscado instanceof Prestamo) {
                ...
                if (materialBuscadoPrestado.prestar() == true) {
        ...
        if (materialBuscado != null) {
            ...
            if (materialBuscado instanceof AccesoDigital) {
                ...
                if (!materialAccesoDigital.sesionActiva()) {
                    ...
                    String nombreBuscado = buscarUsuario(nombreUsuario);

                    if (nombreBuscado != null) {
                        System.out.println(materialAccesoDigital.iniciarSesion(nombreBuscado));
                    } else {
                        System.out.println("El usuario " + nombreUsuario + " no tiene permisos de acceso.");
                    }
                } else {
    ...
    public static String buscarUsuario(String nombreUsuario) {
        String usuario = null;
        for (int i = 0; i < listaUsuarios.size(); i++) {
            if (nombreUsuario.equals(listaUsuarios.get(i))) {
    ...
    public static void renovarMaterial() {
        System.out.println("Introduce el codigo del material:");
        String codigo = miScanner.nextLine().toUpperCase();
        ...
        if (materialBuscado != null) {
            if (materialBuscado instanceof Renovable) {
                Renovable materialBuscadoRenovable = (Renovable) materialBuscado;
                System.out.println(materialBuscadoRenovable.renovar());
            } else {
                System.out.println(materialBuscado.getTitulo() + " no es renovable.");
            }
        } else {
            System.out.println("Material no encontrado.");
        }
